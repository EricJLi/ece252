...
int main(int argc, char **argv) {
    ...
    //Calculate new crc
    crc_val = crc(buffer_def, bufDefCounter); // down cast the return val to U32
    printf("crc_val = %u\n", crc_val);
    ...
    //bufDefCounter
    U64 NbufDefCounter = (U32)htonl(bufDefCounter);
    ...
    //buffer_def
    // for (int x = finLength; x > 0; x--) {
    //     fwrite(&buffer_def[x-1], 1, 1, fout);
    // }
    fwrite(&buffer_def, 1, bufDefCounter, fout);
    fread(&input32, 4, 1, fin);
    //CRC Val
    crc_val = htonl(crc_val);
    printf("CRC input32 %u, crcval %u\n", input32, crc_val);
